...
Обычный `dotnet build` компилирует Core без production-возможности изменять игру. Возможность записи появляется только в специальной Release-сборке, которую создаёт `scripts\publish-windows-preview.ps1`: скрипт передаёт закрытые от произвольной подмены MSBuild-параметры, встраивает исходный точный профиль и границу доверия для режима совместимой ревизии, затем проверяет опубликованный CLI до упаковки.
...
- x64 Windows 10 build 14393 (версия 1607, включая Enterprise 2016 LTSC) или
  новее, либо Windows 11 x64;
- Git;
...
Для 3.1.3 сохраните старый встроенный каталог в паре с его профилем: он нужен для проверки установок, сделанных версией 3.1.2. Актуальный каталог из `translations/` поставляется отдельно через подписанный канал. Извлеките публичный исходный каталог из первоначального коммита, а не из готового EXE:
...
Ожидаемый SHA-256: `78D129B19F4619754BEE92AB876DA24FECC73807032D4A13B27CEAA5EE9619D1`. Не подставляйте свежий каталог в старый профиль и не меняйте закреплённые хэши. После сборки команда `update-refresh --json` загрузит и проверит актуальный подписанный каталог, не изменяя игру.
...
- **Новая версия игры:** сначала проверьте встроенный режим `compatible-revision`. Для поддерживаемого raw LOC1 schema 4 и известной content family обычно достаточно свежего подписанного каталога: точные EN+UK совпадения применятся, остальные строки останутся английскими. Exact-профиль выпускайте, когда нужно полное воспроизводимое покрытие конкретного tuple; EXE пересобирайте только при изменении кода или границы формата/доверия.
...
Удалённый manifest не может заменить встроенный endpoint или ключ. Патчер проверяет подпись, срок, монотонный `sequence`, отозванные выпуски, размеры и SHA-256 Brotli/распакованного каталога, минимальную версию патчера и выбранный exact-профиль либо аутентифицированное семейство, внутри которого локально закрепляется compatible-revision. После удачной проверки данные кладутся в content-addressed cache `%LOCALAPPDATA%\InvokersRu\updates`; при сбое сети используется только ранее проверенный last-known-good либо встроенный bootstrap-каталог/профиль по правилам срока и установленного состояния.
...
`hint_sha256` не заменяет `source_sha256`: в режиме `compatible-revision` обязательны оба точных совпадения. Exact-профиль дополнительно закрепляет весь UK base и итоговый raw LOC1. При подготовке release украинский hint надо передавать валидатору, чтобы изменение контекста не прошло незамеченным.
...
В `compatible-revision` полный наблюдаемый tuple, keyset, выбранный каталог, локально построенный output и counts также закрепляются до транзакции, но строки допускаются по отдельности. Поэтому новая или изменённая строка даёт английский fallback, а не принудительное применение старого русского текста.
...
## 11. Новая версия игры: compatible mode и необязательный exact-профиль
